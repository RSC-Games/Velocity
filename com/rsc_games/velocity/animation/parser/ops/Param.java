package com.rsc_games.velocity.animation.parser.ops;

import java.util.ArrayList;

import com.rsc_games.velocity.animation.parser.Token;
import com.rsc_games.velocity.animation.parser.TokenID;

/** 
 * One of multiple different directives for parsing the animator tree files.
 * This represents an animator parameter (@PARAM). Parameters are "branches"
 * since they envelop operations below them.
 */
public class Param extends Directive {
    /**
     * Create the representation. 
     * 
     * @param opcode Opcode (should be @)
     * @param terminator Terminating character for the line (generally a ;)
     * @param args Args for the opcode (should be PARAM).
     * @param height Nested height? For parsing nested code.
     */
    public Param(Token opcode, Token terminator, ArrayList<Token> args, int height) {
        super(opcode, terminator, args, height);
    }

    /**
     * Determine whether this is a branch or not.
     * Non-branching directives are parsed differently.
     * 
     * @return Whether this is a branch or not.
     */
    @Override
    public boolean isBranch() { return true; }

    /**
     * Validate the args count and type.
     */
    @Override
    protected void verifyArgs() {
        int argCount = 2;
        TokenID[] argType = {TokenID.TOK_TYPE_CODE, TokenID.TOK_TYPE_DATA};

        verifyArgTypeAndCount(argCount, argType);
    }
}
